...
router.post("/message", authenticate, async function (req, res) {
    try {
        ...
        responseChannelTimeout = setTimeout(() => {
            ...
            res.status(500).json({ error: "response channel timeout" });
        }, 3000);
...
router.put("/order/:id_order", authenticate, async function (req, res) {
    try {
        if (!req.params.id_order || isNaN(req.params.id_order)) return res.status(400).json({ error: "missing id_order" });
        if (!req.body.status) return res.status(400).json({ error: "missing status" });
        if (!global.whatsapp_number) return res.status(422).json({ error: "whatsapp not ready" });
        ...
        win.webContents.send("whatsapp:sendOrderMessage", responseChannelId, req.params.id_order, req.body.status);
